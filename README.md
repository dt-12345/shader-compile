...
the program is very crude, it just repeatedly scans `sd:/shaders/` for files and compiles any shaders it finds outputting them to `sd:/output/` with `.bin` appended to the end of the filename (`.control` for control and `.code` for the gpu code/data)
